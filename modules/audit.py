...
class AuditLogger:
    ...
    def _resolve_channels(self, guild: discord.Guild | None) -> list[int]:
        ...
        if guild is not None:
            guild_channel = self.c.audit.guilds.get(
                guild.id,
                self.c.audit.guilds.get(str(guild.id))
            )
    ...
    async def log(
        ...
        *,
        ...
        channel: discord.abc.GuildChannel | discord.abc.PrivateChannel | None = None,
        detail: str = '',
        success: bool = True,
    ):
        '''
        ...
        if guild is not None:
            embed.add_field(name='服务器', value=f'`{guild.name}` (`{guild.id}`)', inline=False)
        if channel is not None:
            channel_repr = getattr(channel, 'mention', None) or getattr(channel, 'name', str(channel))
        ...
        for channel_id in channel_ids:
            try:
                ...
                if not isinstance(target, (discord.TextChannel, discord.Thread)):
                    l.warning(f'[audit] 日志频道 {channel_id} 不是可发送消息的文字频道, 已跳过')
                    continue
